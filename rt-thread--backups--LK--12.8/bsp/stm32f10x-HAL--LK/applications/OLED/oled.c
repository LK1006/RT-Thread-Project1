...
#define OLED_SDA_SET  HAL_GPIO_WritePin(GPIOB,GPIO_PIN_12,GPIO_PIN_SET);
...
#define OLED_SDA_CLR HAL_GPIO_WritePin(GPIOB,GPIO_PIN_12,GPIO_PIN_RESET);
#define OLED_RST_CLR HAL_GPIO_WritePin(GPIOB,GPIO_PIN_15,GPIO_PIN_RESET);
#define OLED_DC_CLR  HAL_GPIO_WritePin(GPIOB,GPIO_PIN_14,GPIO_PIN_RESET);
#define OLED_CS_CLR  HAL_GPIO_WritePin(GPIOC,GPIO_PIN_13,GPIO_PIN_RESET);
...
0xFF,0xFE,0xFE,0xFC,0xFC,0xF8,0xF8,0xF0,0xF0,0xE0,0xE0,0xC0,0xC0,0x80,
0xFF,0x7F,0x7F,0x3F,0x3F,0x1F,0x1F,0x0F,0x0F,0x07,0x07,0x03,0x03,0x01,/*"角",0*/
...
///////// 添加  ////////////
void OLED_WrDat2(unsigned char data)
{
	unsigned char i=8;
	//LCD_CS=0;;
	OLED_DC_SET;
...
//	    GPIO_Set(GPIOE,PIN12|PIN14,GPIO_MODE_OUT,GPIO_OTYPE_PP,GPIO_SPEED_100M,GPIO_PUPD_PU);
  ...
  __HAL_RCC_GPIOC_CLK_ENABLE();           //开启GPIOB时钟
	...
	GPIO_Initure.Pin=GPIO_PIN_12|GPIO_PIN_13|GPIO_PIN_14|GPIO_PIN_15; //PB1,0
  ...
  HAL_GPIO_Init(GPIOB,&GPIO_Initure);
	...
	HAL_GPIO_WritePin(GPIOB,GPIO_PIN_0,GPIO_PIN_SET);						 //PA.8 输出高
	HAL_GPIO_WritePin(GPIOB,GPIO_PIN_1,GPIO_PIN_SET);						 //PA.8 输出高
	HAL_GPIO_WritePin(GPIOB,GPIO_PIN_2,GPIO_PIN_SET);						 //PA.8 输出高
	HAL_GPIO_WritePin(GPIOB,GPIO_PIN_3,GPIO_PIN_SET);						 //PA.8 输出高
	HAL_GPIO_WritePin(GPIOC,GPIO_PIN_13,GPIO_PIN_SET);						 //PA.8 输出高
...
void OLED_PrintU16_3(unsigned char x,unsigned char y,unsigned int num)
{
	...
	tmp[3]=0;
	tmp[2]=(unsigned char)(num%10+0x30);
	tmp[1]=(unsigned char)(num/10%10+0x30);
	tmp[0]=(unsigned char)(num/100%10+0x30);
	...
	OLED_P8x16Str(x,y,tmp);
}

void OLED_PrintU16_2(unsigned char x,unsigned char y,unsigned int num)
{
	unsigned char tmp[3],i;
	tmp[2]=0;
	tmp[1]=(unsigned char)(num%10+0x30);
	tmp[0]=(unsigned char)(num/10%10+0x30);
//	tmp[2]=(unsigned char)(num/100%10+0x30);
//	tmp[1]=(unsigned char)(num/1000%10+0x30);
//	tmp[0]=(unsigned char)(num/10000%10+0x30);
...
void OLED_ShowNum(uint8_t x,uint8_t y,unsigned int num,uint8_t len)
{
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			...
			{
				tmp1[0]=' ';
				OLED_P6x8Str(x+(6/2)*t,y,tmp1);
				continue;
			}else enshow=1; 
		 	 
		}
		tmp2[0]='0'+temp;
	 	OLED_P6x8Str(x+(6/2)*t,y,tmp2); 
	}
}
...
void OLED_PrintFloat(unsigned char x,unsigned char y,float num)
{
    ...
    tmp[4]=(unsigned char)(0x2E);
		tmp[3]=(unsigned char)((int)num/100%10+0x30);
...
/************* 添加 &**************/
void OLED_P8x16Str2(unsigned char x,unsigned char y,unsigned char ch[])
{
  ...
  {    
    ...
    OLED_Set_Pos(x,y);    
  	for(i=0;i<8;i++)     
  	  OLED_WrDat2(F8X16[c*16+i]);
  	OLED_Set_Pos(x,y+1);    
  	for(i=0;i<8;i++)     
  	  OLED_WrDat2(F8X16[c*16+i+8]);  
  	x+=8;
  	j++;
  }
}

void OLED_P14x16Str2(unsigned char x,unsigned char y,unsigned char ch[])
{
